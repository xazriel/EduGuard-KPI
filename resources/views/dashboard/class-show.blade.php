...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="glass-card p-5 lg:col-span-1 flex items-center gap-4">
        <div class="relative w-16 h-16 flex-shrink-0">
            <svg class="w-16 h-16 -rotate-90" viewBox="0 0 36 36">
                ...
                <circle cx="18" cy="18" r="15.9" fill="none"
                    stroke="{{ $overall >= 80 ? '#22c55e' : ($overall >= 60 ? '#eab308' : '#ef4444') }}"
        ...
        <div>
            ...
            <p class="text-xs {{ $overall >= 80 ? 'text-emerald-400' : ($overall >= 60 ? 'text-amber-400' : 'text-red-400') }}">
                {{ $overall >= 80 ? 'Baik' : ($overall >= 60 ? 'Perlu Perhatian' : 'Risiko Tinggi') }}
            </p>
        </div>
    </div>
    @foreach([
    ...
    <div class="glass-card p-4">
        ...
        <p class="text-2xl font-bold {{ $avgKpi[$dim['key']] >= 80 ? 'text-emerald-400' : ($avgKpi[$dim['key']] >= 60 ? 'text-amber-400' : 'text-red-400') }}">
            {{ $avgKpi[$dim['key']] }}
        </p>
        <div class="w-full bg-slate-100 rounded-full h-1.5 mt-2">
            <div class="h-1.5 rounded-full {{ $avgKpi[$dim['key']] >= 80 ? 'bg-emerald-500' : ($avgKpi[$dim['key']] >= 60 ? 'bg-amber-500' : 'bg-red-500') }}"
            ...
            <tbody class="divide-y divide-gray-800/50">
                ...
                <tr class="hover:bg-slate-100/30 transition-colors">
                    ...
                    <td class="py-3 text-center">
                        <span class="font-bold {{ ($student->kpi?->overall_score ?? 100) >= 80 ? 'text-emerald-400' : (($student->kpi?->overall_score ?? 100) >= 60 ? 'text-amber-400' : 'text-red-400') }}">
                            {{ $student->kpi?->overall_score ?? 100 }}
                        </span>
                    </td>
                    <td class="py-3 text-center">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new ApexCharts(document.getElementById('chartCat'), {
        ...base,
        chart: { ...base.chart, type:'donut', height:200 },
